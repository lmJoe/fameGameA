...
export default class questionAndAnswer extends Laya.Dialog {
    ...
    backpage(){
      this.answerFlag = true;
      // console.log("关闭",this.answerFlag);
      Laya.Dialog.open(Scenes.missionSystems);
      // Laya.Dialog.open(Scenes.Spinwin,false);
    }
    getAnswer(index) {
        // code: 1--点击答案   2--过时间
        // console.log("点击",this.answerFlag);
        if (this.answerFlag) {
          var param = {
            qaId: Number(this.issue.id), //	T文本	是	问题id
          ...
          ajax({
            ...
            async: true,
            success:(res)=>{
              // console.log("成功",this.answerFlag,res);
              if (res.code == 1) {
                this.answerFlag = false;
                this.sureCode = res.data;
                this.changeskin(index,res.data);
                // 确定状态
                if (res.data == index) {
                ...
                Laya.timer.loop(1000, this, ()=>{
                  this.second.text = '返回  '+this.secondCode+'s';
                  if (this.secondCode == 0) {
